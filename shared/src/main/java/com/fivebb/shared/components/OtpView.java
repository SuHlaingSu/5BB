...
public class OtpView extends LinearLayout {
  ...
  private void styleEditTexts(TypedArray styles) {
    length = styles.getInt(R.styleable.OtpView_length, 6);
    generateViews(styles);
  }

  private void generateViews(TypedArray styles) {
    if (length > 0) {
      int width = (int) styles.getDimension(R.styleable.OtpView_width, getPixels(40));
      int height = (int) styles.getDimension(R.styleable.OtpView_height, getPixels(40));
      int space = (int) styles.getDimension(R.styleable.OtpView_space, getPixels(0));
      int spaceLeft = (int) styles.getDimension(R.styleable.OtpView_space_left, getPixels(1));
      int spaceRight = (int) styles.getDimension(R.styleable.OtpView_space_right, getPixels(1));
      int spaceTop = (int) styles.getDimension(R.styleable.OtpView_space_top, getPixels(0));
      int spaceBottom = (int) styles.getDimension(R.styleable.OtpView_space_bottom, getPixels(0));
      LayoutParams params =
...
//        editText.setBackgroundResource(R.drawable.edittext_box_with_border);
        editText.setGravity(Gravity.CENTER_HORIZONTAL);
        editText.setMaxLines(1);
        editText.setFilters(filter);
        editText.setLayoutParams(params);
        if (backgroundColor != Color.TRANSPARENT) {
          editText.setBackgroundResource(R.drawable.square_bg_primary_edittext);
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            editText.setElevation(8.0f);
          }
        } else {
